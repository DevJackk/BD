...
download_file('https://raw.githubusercontent.com/jack1eljoker1/BD/master/inc/Run.lua','./inc/Run.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'40% - |█████       |') end
download_file('https://raw.githubusercontent.com/jack1eljoker1/BD/master/inc/locks.lua','./inc/locks.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'60% - |███████     |') end
download_file('https://raw.githubusercontent.com/jack1eljoker1/BD/master/inc/Script.lua','./inc/Script.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'80% - |█████████   |') end
download_file('https://raw.githubusercontent.com/jack1eljoker1/BD/master/inc/functions.lua','./inc/functions.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'100% - |█████████████|\n\n🔝*¦* تم تحديث السورس الى اصدار *v'..redis:get(jack..":VERSION")..'*\n📟*¦* تم اعاده تشغيل السورس بنجاح') end
if edit then dofile("./inc/Run.lua") end
print("Update Source And Reload ~ ./inc/Run.lua")

end
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(jack..":NameBot:") or "الحلم الاسود"
...
local Fshar_Word , res = https.request('https://jack.tk/Words_Fshars.txt')
...
Fshar_Word , res = https.request('https://jack.tk/Words_Fshars.txt')
if res ~= 200 then Fshar_Word = "\n" end
...
var = 'مطور البوت 👨🏽‍💻' 
elseif redis:sismember(jack..':FASSTAS_Group:'..ChatID,UserID) then
var = ' المنشىء اساسي👷🏽' 
elseif redis:sismember(jack..':FASSTAS_BOT:'..ChatID,UserID) then
var = ' المنشىء 👷🏽' 
elseif redis:sismember(jack..'owners:'..ChatID,UserID) then
var = 'مدير البوت 👨🏼‍⚕️' 
elseif redis:sismember(jack..'admins:'..ChatID,UserID) then
var = 'ادمن في البوت 👨🏼‍🎓' 
elseif redis:sismember(jack..'whitelist:'..ChatID,UserID) then
var = 'عضو مميز ⭐️' 
else
var = 'فقط عضو 🙍🏼‍♂️' 
end
...
Replay = Replay:gsub("{البوت}",redis:get(jack..':NameBot:'))
...
if msg.edited then
redis:incr(jack..':edited:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.text and not msg.forward_info_ then
redis:incr(jack..'msgs:'..msg.sender_user_id_..':'..msg.chat_id_)
elseif msg.content_.ID == "MessageChatAddMembers" then 
redis:incr(jack..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessagePhoto" then
redis:incr(jack..':photo:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageSticker" then
redis:incr(jack..':sticker:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageVoice" then
redis:incr(jack..':voice:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageAudio" then
redis:incr(jack..':audio:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageVideo" then
redis:incr(jack..':video:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageAnimation" then
redis:incr(jack..':animation:'..msg.chat_id_..':'..msg.sender_user_id_)
end
end
...
local monshaas = redis:smembers(jack..':FASSTAS_Group:'..msg.chat_id_)
local monsha = redis:smembers(jack..':FASSTAS_BOT:'..msg.chat_id_)
local Owners = redis:smembers(jack..'owners:'..msg.chat_id_)
local Admins = redis:smembers(jack..'admins:'..msg.chat_id_)
local mmez = redis:smembers(jack..'whitelist:'..msg.chat_id_)
...
local info  = redis:hgetall(jack..'username:'..v)
...
local list = redis:smembers(jack..':SUDO_BOT:')
message = '👨🏽‍💻*¦* قائمه الـمـطـوريـن : \n\n`★`*_* ['..SUDO_USER..'] ➣ (' ..SUDO_ID.. '){'..redis:scard(jack..'mtwr_count'..SUDO_ID)..'}\n*----------------------------------*\n'
if #list==0 then  message = message.."* لا يوجد مطورين حاليا \n📛 *"
else
for k,v in pairs(list) do
local info  = redis:hgetall(jack..'username:'..v)
local count = redis:scard(jack..'mtwr_count'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
...
local list = redis:smembers(jack..'owners:'..msg.chat_id_)
if #list == 0 then  
...
local list = redis:smembers(jack..'admins:'..msg.chat_id_)
...
local list = redis:smembers(jack..'whitelist:'..msg.chat_id_)
...
if redis:sismember(jack..'is_silent_users:'..Group,User) then 
return true 
else
return false
end 
...
local list = redis:smembers(jack..'is_silent_users:'..msg.chat_id_)
...
if redis:sismember(jack..'banned:'..Group,User) then 
return true 
else
return false
end 
...
if redis:sismember(jack..'gban_users',User) then 
return true 
else
return false
end 
...
local list = redis:smembers(jack..'gban_users')
...
local list = redis:smembers(jack..':Filter_Word:'..msg.chat_id_)
if #list ~=0 then
...
if redis:sismember(jack..':Filter_Word:'..msg.chat_id_,word) then 
return  "📝*¦* الكلمه *{"..word.."}* هي بالتأكيد من قائمه المنع✓️" 
else
redis:sadd(jack..':Filter_Word:'..msg.chat_id_,word) 
...
if redis:sismember(jack..':Filter_Word:'..msg.chat_id_,word) then 
redis:srem(jack..':Filter_Word:'..msg.chat_id_,word) 
...
local NumberFlood = tonumber(redis:get(jack..':Flood_Spam:'..User..':'..ChatID..':msgs') or 0)
if NumberFlood >= 5 then 
result = false
else
redis:setex(jack..':Flood_Spam:'..User..':'..ChatID..':msgs',2,NumberFlood+1)
result = true
end
return result
end
...
json_data = '{"BotID": '..jack..',"UserBot": "'..Bot_User..'","Groups" : {'
local All_Groups_ID = redis:smembers(jack..'group:ids')
for key,GroupS in pairs(All_Groups_ID) do
local NameGroup = (redis:get(jack..'group:name'..GroupS) or '')
...
local admins = redis:smembers(jack..'admins:'..GroupS)
if #admins ~= 0 then
json_data =  json_data..',"Admins" : {'
for key,value in pairs(admins) do
local info = redis:hgetall(jack..'username:'..value)
if info then 
...
local creator = redis:smembers(jack..':FASSTAS_BOT:'..GroupS)
if #creator ~= 0 then
...
local owner = redis:smembers(jack..'owners:'..GroupS)
if #owner ~= 0 then
...
for k,v in pairs(list) do 
local info = redis:get(jack..'group:name'..v)
if info then 
...
redis:del(
jack..'group:add'..id_group,
jack..'lock_link'..id_group, 
jack..'lock_id'..id_group,
jack..'lock_spam'..id_group, 
jack..'lock_webpage'..id_group,
jack..'lock_markdown'..id_group,
jack..'lock_flood'..id_group,
jack..'lock_bots'..id_group,
jack..'mute_forward'..id_group,
jack..'mute_contact'..id_group,
jack..'mute_location'..id_group,
jack..'mute_document'..id_group,
jack..'mute_keyboard'..id_group,
jack..'mute_game'..id_group,
jack..'mute_inline'..id_group,
jack..'lock_username'..id_group,
jack..'num_msg_max'..id_group,
jack..'mute_text'..id_group,
jack..'admins:'..id_group,
jack..':Filter_Word:'..id_group,
jack..'banned:'..id_group,
jack..'is_silent_users:'..id_group,
jack..'whitelist:'..id_group,
jack..':FASSTAS_BOT:'..id_group,
jack..'owners:'..id_group,
jack..'replay'..id_group,
jack..':FASSTAS_Group:'..id_group
)
redis:srem(jack..'group:ids',id_group)
end
...
NumAdmin = NumAdmin + 1
if not redis:sismember(jack..'admins:'..arg.chat_id_,v.user_id_) then
GetUserID(v.user_id_,function(arg,data)
redis:hset(jack..'username:'..data.id_,'username',ResolveUserName(data))
redis:sadd(jack..'admins:'..arg.chat_id_,data.id_)
...
function modadd(msg)
if redis:get(jack..'lock_service') then
lock_servicez = true
else
lock_servicez = false
end
...
if redis:get(jack..'group:add'..msg.chat_id_) then  return sendMsg(msg.chat_id_,msg.id_,'📮¦ بواسطه ⋙「 '..NameUser..' 」 \n📬¦ المجموعه بالتأكيد ✓️ تم تفعيلها \n✓️') end
...
local GroupUsers = tonumber(redis:get(jack..':addnumberusers') or 0)
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(jack..'username:'..data.id_,'username', ResolveUserName(data))
redis:sadd(jack..':FASSTAS_Group:'..arg.chat_id_,data.id_)
...
redis:mset(
jack..'group:add'..arg.chat_id_,true,
jack..'lock_link'..arg.chat_id_,true, 
jack..'lock_id'..arg.chat_id_,true,
jack..'lock_spam'..arg.chat_id_,true,
jack..'lock_webpage'..arg.chat_id_,true,
jack..'lock_markdown'..arg.chat_id_,true,
jack..'lock_flood'..arg.chat_id_,true,
jack..'lock_bots'..arg.chat_id_,true,
jack..'mute_forward'..arg.chat_id_,true,
jack..'mute_contact'..arg.chat_id_,true,
jack..'mute_location'..arg.chat_id_,true,
jack..'mute_document'..arg.chat_id_,true,
jack..'mute_keyboard'..arg.chat_id_,true,
jack..'mute_game'..arg.chat_id_,true,
jack..'mute_inline'..arg.chat_id_,true,
jack..'lock_username'..arg.chat_id_,true,
jack..'num_msg_max'..arg.chat_id_,5, 
jack..'lock_edit'..arg.chat_id_,true,
jack..'replay'..arg.chat_id_,true,
jack..'lock_rdodSource'..arg.chat_id_,true,
jack.."lock_KickBan"..msg.chat_id_,true,
jack.."lock_mmno3"..msg.chat_id_,true,
jack.."lock_KickBan"..msg.chat_id_,true,
jack.."lock_linkk"..msg.chat_id_,true
)
redis:sadd(jack..'group:ids',arg.chat_id_) 
redis:sadd(jack..'mtwr_count'..arg.sender_user_id_,arg.chat_id_)

local NameGroup = data.title_
redis:set(jack..'group:name'..arg.chat_id_,NameGroup)
...
redis:set(jack..'linkGroup'..arg.chat_id_,Gp_Link)
...
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:sadd(jack..':tqeed:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n⛔️¦ تم تقييده بنجاح \n✓")
end 
if cmd =="fktqeed" then
Restrict(ChatID,UserID,2)
redis:srem(jack..':tqeed:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n⛔️¦ تم فك تقييده بنجاح \n✓")
end
if cmd == "setwhitelist" then
if redis:sismember(jack..'whitelist:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد ترقيته مميز  في المجموعه \n✓") 
end
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:sadd(jack..'whitelist:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم ترقيته مميز  في المجموعه \n✓") 
end
if cmd == "remwhitelist" then
if not redis:sismember(jack..'whitelist:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد تنزيله مميز  في المجموعه \n✓") 
end
redis:srem(jack..'whitelist:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم تنزيله مميز  في المجموعه \n✓") 
end
if cmd == "setmnsha" then
if redis:sismember(jack..':FASSTAS_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد ترقيته منشئ  في المجموعه \n✓") 
end
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:sadd(jack..':FASSTAS_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم ترقيته منشئ  في المجموعه \n✓") 
end
if cmd == "remmnsha" then
if not redis:sismember(jack..':FASSTAS_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد تنزيله منشئ  في المجموعه \n✓") 
end
redis:srem(jack..':FASSTAS_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم تنزيله منشى  في المجموعه \n✓") 
end

if cmd == "setowner" then
if redis:sismember(jack..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد ترقيته مدير  في المجموعه \n✓") 
end
Resolv = Resolv:gsub([[\_]],"_")
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:sadd(jack..'owners:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم ترقيته مدير  في المجموعه \n✓") 
end
if cmd == "remowner" then
if not redis:sismember(jack..'owners:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد تنزيله مدير  في المجموعه \n✓") 
end
redis:srem(jack..'owners:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم تنزيله مدير  في المجموعه \n✓") 
end
if cmd == "promote" then
if redis:sismember(jack..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد ترقيته ادمن  في المجموعه \n✓") 
end
Resolv = Resolv:gsub([[\_]],"_")
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:sadd(jack..'admins:'..ChatID,UserID) 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم ترقيته ادمن  في المجموعه \n✓") 
end
if cmd == "demote" then
if not redis:sismember(jack..'admins:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد تنزيله ادمن  في المجموعه \n✓") 
end
redis:srem(jack..'admins:'..ChatID,UserID)
...
if cmd == "Upmonsh" then
if redis:sismember(jack..':FASSTAS_Group:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد ترقيته منشئ اساسي  في المجموعه \n✓") 
end
redis:hset(jack..'username:'..UserID,'username',USERNAME)
redis:sadd(jack..':FASSTAS_Group:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم ترقيته منشئ اساسي  في المجموعه \n✓") 
end

if cmd == "Dwmonsh" then
if not redis:sismember(jack..':FASSTAS_Group:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n📋¦ تم بالتأكيد تنزيله منشئ اساسي  في المجموعه \n✓") 
end
redis:srem(jack..':FASSTAS_Group:'..ChatID,UserID) 
...
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:sadd(jack..'banned:'..ChatID,UserID)
...
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:srem(jack..'banned:'..ChatID,UserID)
StatusLeft(ChatID,UserID)
return false
end
...
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:sadd(jack..'is_silent_users:'..ChatID,UserID)
...
redis:hset(jack..'username:'..UserID, 'username', Resolv)
redis:srem(jack..'is_silent_users:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮¦ المستخدم  ⋙「 "..NameUser.." 」 \n⛔️¦ تم الغاء كتمه  من المجموعه \n✓") 
end

if cmd == "upMshrf" then
redis:hset(jack..'username:'..UserID,'username',Resolv)
redis:setex(jack..":uploadingsomeon:"..ChatID..msg.sender_user_id_,500,NameUser)
redis:setex(jack..":uploadingsomeon2:"..ChatID..msg.sender_user_id_,500,UserID)
...
if cmd == "tfa3l" then  
local maseegs = redis:get(jack..'msgs:'..UserID..':'..ChatID) or 1
local edited = redis:get(jack..':edited:'..ChatID..':'..UserID) or 0
local content = redis:get(jack..':adduser:'..ChatID..':'..UserID) or 0
if data.username_ then UserNameID = "@"..data.username_ else UserNameID = "لا يوجد" end  
...
.."\n📝¦ التعديل » "..(redis:get(jack..'lock_edit'..msg.chat_id_) or 'false')
.."\n🔗¦ الروابط » "..(redis:get(jack..'lock_link'..msg.chat_id_) or 'false')
.."\n#️⃣¦ التاك » "..(redis:get(jack..'lock_tag'..msg.chat_id_) or 'false')
.."\n©¦ المعرفات » "..(redis:get(jack..'lock_username'..msg.chat_id_) or 'false')
.."\n\n💱¦ التكرار » "..(redis:get(jack..'lock_flood'..msg.chat_id_) or 'false')
.."\n📑¦ الكلايش » "..(redis:get(jack..'lock_spam'..msg.chat_id_) or 'false')
.."\n🌐¦ الويب » "..(redis:get(jack..'lock_webpage'..msg.chat_id_) or 'false')
.."\n⚜️¦ الماركدوان » "..(redis:get(jack..'lock_markdown'..msg.chat_id_) or 'false')
.."\n🏌🏻¦ البوتات بالطرد » "..(redis:get(jack..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n🤖¦ البوتات » "..(redis:get(jack..'lock_bots'..msg.chat_id_) or 'false')
.."\n➕¦ عدد التكرار » "..(redis:get(jack..'num_msg_max'..msg.chat_id_) or 'false')
.."\n⏰¦ وقت التنظيف » "..(redis:get(jack..':Timer_Cleaner:'..msg.chat_id_) or '6').." ساعة ."
.."\n\n🎬¦` اعدادات الوسائط :`\n"
.."\n🤹*¦* المتحركه » "..(redis:get(jack..'mute_gif'..msg.chat_id_) or 'false')
.."\n💭¦ الدردشه » "..(redis:get(jack..'mute_text'..msg.chat_id_) or 'false')
.."\n◽️¦ الانلاين » "..(redis:get(jack..'mute_inline'..msg.chat_id_) or 'false')
.."\n🎮¦ الالعاب » "..(redis:get(jack..'mute_game'..msg.chat_id_) or 'false')
.."\n🏞¦ الصور » "..(redis:get(jack..'mute_photo'..msg.chat_id_) or 'false')
.."\n🎥¦ الفيديو » "..(redis:get(jack..'mute_video'..msg.chat_id_) or 'false')
.."\n🎙¦ الصوت » "..(redis:get(jack..'mute_audio'..msg.chat_id_) or 'false')
.."\n\n🔉¦ البصمات » "..(redis:get(jack..'mute_voice'..msg.chat_id_) or 'false')
.."\n🎎¦ الملصقات » "..(redis:get(jack..'mute_sticker'..msg.chat_id_) or 'false')
.."\n📞¦ الجهات » "..(redis:get(jack..'mute_contact'..msg.chat_id_) or 'false')
.."\n💱¦ التوجيه » "..(redis:get(jack..'mute_forward'..msg.chat_id_) or 'false')
.."\n🌐¦ الموقع » "..(redis:get(jack..'mute_location'..msg.chat_id_) or 'false')
.."\n🗂¦ الملفات » "..(redis:get(jack..'mute_document'..msg.chat_id_) or 'false')
.."\n🔅¦ الاشعارات » "..(redis:get(jack..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n🔒¦ الفشار » "..(redis:get(jack..'lock_mmno3'..msg.chat_id_) or 'false')
.."\n🔒¦ الفارسيه » "..(redis:get(jack..'lock_pharsi'..msg.chat_id_) or 'false')
.."\n🔒¦ الانكليزيه » "..(redis:get(jack..'lock_lang'..msg.chat_id_) or 'false')
.."\n🔒¦ الاضافه » "..(redis:get(jack..'lock_Add'..msg.chat_id_) or 'false')

local eueuf = "\n\n*⚒¦*` اعدادات اخرى : `"
.."\n*🙋🏼‍♂️¦* الترحيب » "..(redis:get(jack..'welcome:get'..msg.chat_id_) or 'false')
.."\n*📋¦*  الردود » "..(redis:get(jack..'replay'..msg.chat_id_) or 'false')
.."\n*🚸¦*  التحذير » "..(redis:get(jack..'lock_woring'..msg.chat_id_) or 'false')
.."\n*📝¦* الايدي » "..(redis:get(jack..'lock_id'..msg.chat_id_) or 'false')
.."\n*💬¦* الرابط » "..(redis:get(jack..'lock_linkk'..msg.chat_id_) or 'false')
.."\n*📮¦* المغادره » "..(redis:get(jack..'lock_leftgroup'..msg.chat_id_) or 'false')
.."\n*🗳¦* الحظر » "..(redis:get(jack..'lock_KickBan'..msg.chat_id_) or 'false')
.."\n*🔍¦* الحمايه » "..(redis:get(jack..'antiedit'..msg.chat_id_) or 'false')
.."\n*📋¦* التاك للكل » "..(redis:get(jack..'lock_takkl'..msg.chat_id_) or 'false')
.."\n*📒¦* الايدي بالصوره » "..(redis:get(jack..'idphoto'..msg.chat_id_) or 'false')
.."\n*🔐¦* التحقق » "..(redis:get(jack.."lock_check"..msg.chat_id_) or 'false')
.."\n*🗑¦* التنظيف التلقائي » "..(redis:get(jack.."lock_cleaner"..msg.chat_id_) or 'false')
.."\n*📂¦* ردود السورس » "..(redis:get(jack.."lock_rdodSource"..msg.chat_id_) or 'false')
...
.."\n*➕¦* عدد التكرار » "..(redis:get(jack..'num_msg_max'..msg.chat_id_) or 'false')
.."\n\n*💱¦*` اعدادات التقـييد :`\n"
.."\n*💢¦* التقييد بالتوجيه » "..(redis:get(jack..':tqeed_fwd:'..msg.chat_id_) or 'false')
.."\n*📸¦* التقييد بالصور » "..(redis:get(jack..':tqeed_photo:'..msg.chat_id_) or 'false')
.."\n*🔗¦* التقييد بالروابط » "..(redis:get(jack..':tqeed_link:'..msg.chat_id_) or 'false')
.."\n*🎉¦* التقييد بالمتحركه » "..(redis:get(jack..':tqeed_gif:'..msg.chat_id_) or 'false')
.."\n*🎥¦* التقييد الفيديو » "..(redis:get(jack..':tqeed_video:'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '{ مقفول }')
list_settings = list_settings:gsub('false', '{ مفتوح }')
...
.."\n*🗂¦* الملفات » "..(redis:get(jack..'mute_document'..msg.chat_id_) or 'false')
.."\n*🔅¦* الاشعارات » "..(redis:get(jack..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n*🔒¦* الكيبورد » "..(redis:get(jack..'mute_keyboard'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '{ مقفول }')
list_settings = list_settings:gsub('false', '{ مفتوح }')
return sendMsg(msg.chat_id_,msg.id_,'➖\n'..list_settings..'\n')
end
