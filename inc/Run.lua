...
local GetVerisonBD = https.request('https://raw.githubusercontent.com/jack1eljoker1/BDFiles/master/GetVersion.txt') or 0
print('\n\27[1;36m￤تم آدخآل آيدي آلمـطـور بنجآح , سـوف يتم تشـغيل آلسـورس آلآن .\n￤Success Save ID : \27[0;32m['..SUDO_USER..']\n\27[0;39;49m')
jack = Token:match("(%d+)")
redis:mset(
jack..":VERSION",GetVerisonBD,
jack..":SUDO_ID:",SUDO_USER,
jack..":DataCenter:","Amsterdam",
jack..":UserNameBot:",BOT_User,
jack..":ApiSource","https://jack.tk/",
jack..":NameBot:","الحلم الاسود",
"BD_INSTALL","Yes"
)
redis:hset(jack..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
info = {} 
...
jack = Token:match("(%d+)")
our_id = tonumber(jack)
ApiBD = redis:get(jack..":ApiSource")
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(jack..":UserNameBot:")
SUDO_ID = tonumber(redis:get(jack..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_USER = redis:hgetall(jack..'username:'..SUDO_ID).username
version = redis:get(jack..":VERSION")
DataCenter = redis:get(jack..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..jack..')\027[0;32m\n'
...
..'¦ Api_Src->: \27[1;34m'..ApiBD..'\027[0;32m\n'
...
if not redis:get(jack..":Check_Bot:"..W) then
Rgz,res=https.request(ApiBD..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(jack..":Check_Bot:"..W,100800,true) return false else return Rgz end 
end 
end
...
if msg.type == "pv" and redis:get(jack..':mute_pv:'..msg.sender_user_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end
if msg.type ~= "pv" and redis:get(jack..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
print("\27[1;31mThis Flood Sender ...\27[0")
Del_msg(msg.chat_id_,msg.id_)
return false
end
if redis:get(jack..'group:add'..msg.chat_id_) then 
...
and redis:get(jack.."lock_cleaner"..msg.chat_id_) then
print("Clener >>> ")
redis:sadd(jack..":IdsMsgsCleaner:"..msg.chat_id_,msg.id_)
Timerr = redis:get(jack..':Timer_Cleaner:'..msg.chat_id_)
if Timerr then 
Timerr = tonumber(Timerr)
Timerr = 60*60*Timerr
end
redis:setex(jack..":SetTimerCleaner:"..msg.chat_id_..msg.id_,Timerr or 21600,true)  
end



print(jack..":IdsMsgsCleaner:"..msg.chat_id_)
local Cleaner = redis:smembers(jack..":IdsMsgsCleaner:"..msg.chat_id_)
for k,v in pairs(Cleaner) do
if not redis:get(jack..":SetTimerCleaner:"..msg.chat_id_..v) then
Del_msg(msg.chat_id_,v)
redis:srem(jack..":IdsMsgsCleaner:"..msg.chat_id_,v)
...
if msg.GroupActive and redis:get(jack..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end
...
msg.Rank = 1
elseif redis:sismember(jack..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(jack..':FASSTAS_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'منشئ اساسي'
msg.TheRank = 'منشئ اساسي 👲🏼'
msg.Rank = 11
elseif msg.GroupActive and redis:sismember(jack..':FASSTAS_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(jack..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(jack..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(jack..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(jack..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(jack..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(jack..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(jack..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(jack..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(jack..':NameBot:')..[[ 🎖
...
if not msg.Special and not msg.Admin and redis:get(jack.."lock_Add"..msg.chat_id_) then
...
if redis:get(jack..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
else
if redis:get(jack.."lock_check"..msg.chat_id_) and not redis:get(jack..":TqeedUser:"..msg.chat_id_..Senderid) then
...
if msg.Special or msg.Admin then redis:srem(jack..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(jack..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(jack..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.BD then 
if msg.text and ScriptFile.iBD then
XBD = ScriptFile.BD
local list = redis:hgetall(jack..":AwamerBotArray:"..msg.chat_id_)
for BD2,k in pairs(list) do
Text = msg.text
Text2 = k
if Text:match(BD2) then 
local amrr = {Text:match(BD2)}
local AmrOld = redis:hgetall(jack..":AwamerBotArray2:"..msg.chat_id_)
amrnew = "" amrold = ""
...
GetMsg = ScriptFile.iBD(msg,{Text:match(Text2)})
if GetMsg then
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(BD) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.iBD(msg,{Text:match(BD)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",BD.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
...
if ScriptFile.dBD then
if ScriptFile.dBD(msg) == false then
return false
end
...
if Plug.BD then 
if msg.text and Plug.iBD then
for k, BD in pairs(Plug.BD) do
if msg.text:match(BD) then
local GetMsg = Plug.iBD(msg,{msg.text:match(BD)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",BD.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source BD")
end 

end
else
print("The File Script.lua Not Runing in The Source BD")
end
end
...
redis:srem(jack..'users',msg.sender_user_id_)
end
...
if redis:get(jack..":propin"..msg.chat_id_) == msg.content_.text_ then
redis:del(jack..":propin"..msg.chat_id_)
...
if ApiBD ~= "https://jack.tk/" then
redis:set(jack..":ApiSource","https://jack.tk/")
end
...
if redis:get(jack..'group:add'..data.chat_.id_) then
redis:set(jack..'group:name'..data.chat_.id_,data.chat_.title_)
end
elseif data.ID == "UpdateChannel" then  
if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
if redis:get(jack..'group:add-100'..data.channel_.id_) then
local linkGroup = (redis:get(jack..'linkGroup-100'..data.channel_.id_) or "")
local NameGroup = (redis:get(jack..'group:name-100'..data.channel_.id_) or "")
...
redis:mset(
jack..'group:name'..k,IDS.Title,
jack..'num_msg_max'..k,5,
jack..'group:add'..k,true,
jack..'lock_link'..k,true,
jack..'lock_spam'..k,true,
jack..'lock_webpage'..k,true,
jack..'lock_markdown'..k,true,
jack..'lock_flood'..k,true,
jack..'lock_bots'..k,true,
jack..'mute_forward'..k,true,
jack..'mute_contact'..k,true,
jack..'mute_document'..k,true,
jack..'mute_inline'..k,true,
jack..'lock_username'..k,true,
jack..'replay'..k,true
)
redis:sadd(jack..'group:ids',k) 

if IDS.Admins then
for user,ID in pairs(IDS.Admins) do
redis:hset(jack..'username:'..ID,'username',user)
redis:sadd(jack..'admins:'..k,ID)
end
end
if IDS.Creator then
for user,ID in pairs(IDS.Creator) do
redis:hset(jack..'username:'..ID,'username',user)
redis:sadd(jack..':FASSTAS_BOT:'..k,ID)
end
end
if IDS.Owner then
for user,ID in pairs(IDS.Owner) do
redis:hset(jack..'username:'..ID,'username',user)
redis:sadd(jack..'owners:'..k,ID)
end
end
end
io.popen("rm -fr ../.telegram-cli/data/document/*")
sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(jack..'group:ids')..'* 🌿\n✓')
end
elseif data.ID == "UpdateUser" then  
if data.user_.type_.ID == "UserTypeDeleted" then
print("¦ userTypeDeleted")
redis:srem(jack..'users',data.user_.id_)
elseif data.user_.type_.ID == "UserTypeGeneral" then
local CheckUser = redis:hgetall(jack..'username:'..data.user_.id_).username
...
redis:hset(jack..'username:'..data.user_.id_,'username',USERNAME)
end 
end
